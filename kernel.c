#include "screen.h"
#include "gui.h"  // Yeni yazdığımız efsane arayüz modülünü buraya bağlıyoruz
#include <stdint.h>

// FARE SÜRÜCÜSÜ DEĞİŞKENLERİ
int mouse_x = 512; 
int mouse_y = 384; 
...
// Donanımsal Fare Polling ve Ekran Güncelleme
void handle_mouse_polling(void) {
    ...
    if ((status & 0x01) && (status & 0x20)) {
        ...
        if (mouse_cycle == 3) { 
            ...
            // Grafik arayüzünü yeni koordinatlarla gui.h üzerinden tazele!
...
void kernel_main(void) {
    // İlk açılışta masaüstünü gui kütüphanesiyle çizdiriyoruz
    gui_refresh_desktop(mouse_x, mouse_y); 
    init_mouse();          
