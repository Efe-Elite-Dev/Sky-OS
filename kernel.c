...
// ====================================================================
// GÜVENLİ GRAFİK MODU VE SİSTEM TANIMLAMALARI (800x600)
// ====================================================================
...
// Modern Pencere Yapısı (800x600 için İdeal Geometri)
typedef struct {
    int x;
    int y;
    int width;
...
void gui_refresh_desktop(int mx, int my, uint32_t tick) {
    ...
    if (active_window != 0) {
        ...
        if (active_window == 1) gui_draw_rect(app_window.x + 15, app_window.y + 50, 420, 255, 0xF8FAFC);
        else if (active_window == 2) gui_draw_rect(app_window.x + 15, app_window.y + 50, 420, 255, 0x0F172A);
        else if (active_window == 3) gui_draw_rect(app_window.x + 15, app_window.y + 50, 420, 255, 0x050515);
    }
...
void handle_desktop_click(int is_pressed) {
    if (active_window != 0) {
        if (mouse_x >= (app_window.x + app_window.width - 44) && mouse_x <= (app_window.x + app_window.width - 8) && mouse_y >= (app_window.y + 6) && mouse_y <= (app_window.y + 30)) {
            if (is_pressed) { active_window = 0; app_window.is_dragging = 0; return; }
        }
        if (mouse_x >= app_window.x && mouse_x <= (app_window.x + app_window.width) && mouse_y >= app_window.y && mouse_y <= (app_window.y + 36)) {
    ...
    if (!is_pressed) app_window.is_dragging = 0;
